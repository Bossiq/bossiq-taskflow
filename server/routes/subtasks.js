...
async function logActivity(taskId, action, details = '') {
  try {
    await db.prepare('INSERT INTO activity_log (task_id, action, details) VALUES (?, ?, ?)').run(taskId, action, details);
  } catch { /* non-critical */ }
}
...
router.post('/', async (req, res) => {
  try {
    ...
    await logActivity(req.params.taskId, 'subtask_added', `Added subtask "${subtask.title}"`);
...
router.patch('/:subtaskId/toggle', async (req, res) => {
  try {
    const existing = await db.prepare('SELECT * FROM subtasks WHERE id = ?').get(req.params.subtaskId);
    if (!existing) return res.status(404).json({ error: 'Subtask not found' });

    await db.prepare('UPDATE subtasks SET completed = ? WHERE id = ?')
      .run(existing.completed ? 0 : 1, req.params.subtaskId);

    const updated = await db.prepare('SELECT * FROM subtasks WHERE id = ?').get(req.params.subtaskId);
    if (updated.completed) {
      await logActivity(req.params.taskId, 'subtask_completed', `Completed subtask "${updated.title}"`);
    }
    res.json(updated);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.put('/:subtaskId', async (req, res) => {
  try {
    ...
    await db.prepare('UPDATE subtasks SET title = ? WHERE id = ?').run(title.trim(), req.params.subtaskId);
    const updated = await db.prepare('SELECT * FROM subtasks WHERE id = ?').get(req.params.subtaskId);
    res.json(updated);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.delete('/:subtaskId', async (req, res) => {
  try {
    const result = await db.prepare('DELETE FROM subtasks WHERE id = ?').run(req.params.subtaskId);
